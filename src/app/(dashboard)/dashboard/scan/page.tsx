    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 relative">
      ...
      {state === "idle" || state === "error" ? (
        ...
        <div className="space-y-12 animate-in fade-in slide-in-from-bottom-10 duration-700">
          ...
          {/* Ghost-Charge Guard (Langganan Siluman) */}
          {result.recurring_charges && result.recurring_charges.length > 0 && (
            <GlassCard className="p-10 border-indigo-500/20 shadow-[0_0_30px_rgba(99,102,241,0.05)]">
              <h4 className="text-xl font-bold text-white mb-2 flex items-center gap-3">
                <Ghost className="w-6 h-6 text-indigo-400" />
                Ghost-Charge Guard
              </h4>
              <p className="text-white/50 text-sm mb-6">AI mendeteksi pengeluaran berulang (langganan) yang mungkin menyedot uang Anda diam-diam.</p>
                  ...
                  <div key={i} className={`p-5 rounded-2xl border ${charge.is_suspicious ? 'bg-red-500/10 border-red-500/30 shadow-[0_0_15px_rgba(239,68,68,0.1)]' : 'bg-[#0B0A08]/50 border-white/5'}`}>
                    <div className="flex justify-between items-start mb-3">
                      ...
                      {charge.is_suspicious && (
                        <span className="text-[10px] uppercase font-bold bg-red-500/20 text-red-400 px-2 py-1 rounded-md shrink-0">Mencurigakan</span>
                      )}
                    </div>
                    <div className="text-2xl font-black text-white mb-1">
                      Rp {charge.amount.toLocaleString("id-ID")}
                    </div>
                    <p className="text-white/40 text-sm capitalize">Siklus: {charge.frequency}</p>
                  </div>
                ))}
              </div>
            </GlassCard>
