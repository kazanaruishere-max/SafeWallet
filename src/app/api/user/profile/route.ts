...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const allowedFields = ["monthly_income", "phone", "onboarding_completed", "birth_year"];
    ...
    for (const field of allowedFields) {
      if (field in body) {
        // Validasi khusus untuk birth_year
        if (field === "birth_year") {
          const year = parseInt(body[field]);
          // Asumsi: Umur manusia tertua ~120 tahun, dan paling muda pengguna aplikasi 5 tahun.
          if (isNaN(year) || year < currentYear - 120 || year > currentYear - 5) {
            return NextResponse.json(
              { success: false, error: { code: "VALIDATION_ERROR", message: "Tahun lahir tidak valid atau tidak masuk akal." } } satisfies ApiError,
              { status: 400 }
            );
          }
          updates[field] = year;
