...
// SECURITY: No fallback allowed — must be set explicitly in production.
// Generate a secure key: node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"
function getEncryptionKey(): string {
  const key = process.env.ENCRYPTION_KEY;
  if (!key || key.length < 32) {
    throw new Error(
      "[SafeWallet] ENCRYPTION_KEY environment variable is not set or is too short (minimum 32 characters). " +
